...
export function DosResult() {
  ...
  // Get result data from navigation state (passed from DosTest)
  ...
  // Get additional data from local dosResultTypes
  const localResultData = dosResultTypes.find((rt) => rt.id === type);

  // Parse detail, features, and tags
  const detail = localResultData?.detail || [];
  const features = localResultData?.features || [];
  const tags = localResultData?.tag || [];
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 sm:py-12">
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2 sm:mb-3 gap-1">
                    ...
                    <span className="text-xs sm:text-sm text-gray-400">
                      {axis.left} {axis.leftScore}% · {axis.rightScore}%{" "}
                      {axis.right}
                    </span>
                  </div>
        ...
        >
          ...
          {detail.length > 0 ? (
            <div className="text-gray-300 text-sm sm:text-base leading-relaxed space-y-3 sm:space-y-4">
              {detail.map((section: string, index: number) => (
        ...
        >
          ...
          <p className="text-xs sm:text-sm text-gray-300 leading-relaxed">
            ...
            다양한 요소로 구성되며, 시간에 따라 변할 수 있습니다. 이 테스트는
            자신의 정치적 위치를 이해하는 참고 자료로만 활용해주세요.
          </p>
        </motion.div>
