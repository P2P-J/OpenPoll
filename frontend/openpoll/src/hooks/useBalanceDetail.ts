...
import type { UseBalanceDetailResult, VoteOption, VoteState } from "@/types/balanceDetail.types";
...
} from "@/utils/balanceDetailHelpers";

function ensureCommentsShape(issue: BalanceDetail): BalanceDetail {
  const comments = (
    (issue as BalanceDetail & { comments?: BalanceCommentNodeModel[] }).comments ?? []
  ) as BalanceCommentNodeModel[];
...
export function useBalanceDetail(id?: string): UseBalanceDetailResult {
  ...
  const comments = useMemo<BalanceCommentNodeModel[]>(
    () =>
      issue
        ? (((issue as BalanceDetail & { comments?: BalanceCommentNodeModel[] }).comments ??
            []) as BalanceCommentNodeModel[])
  ...
  const isMyComment = (c: BalanceCommentNodeModel): boolean => {
  ...
  const refreshComments = async (issueId: number) => {
    const commentList = (await issueApi.getComments(issueId)) as BalanceCommentNodeModel[];
    setIssue((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
    ...
    const run = async () => {
      try {
        ...
        const [detail, commentList] = await Promise.all([
          issueApi.getIssueDetail(issueId),
        ...
        setIssue(merged);
        setSelectedOption(
          isLoggedInNow() ? fromMyVote((merged as { myVote?: boolean | null }).myVote) : null
        );
      } catch (e) {
        if (!mounted) return;
  ...
  const handleVote = async (option: VoteOption) => {
    ...
    try {
      setIsVoting(true);
      setErrorMessage(null);
      await issueApi.voteIssue(prevIssue.id, toApiVote(option));
    } catch (e) {
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      await issueApi.createComment(issue.id, { content });
  ...
  const handleDeleteComment = async (commentId: string | number) => {
    ...
    try {
      await issueApi.deleteComment(issue.id, commentIdNum);
      await refreshComments(issue.id);
  ...
  const handleSubmitReply = async (parentId: string | number) => {
    ...
    try {
      await issueApi.createComment(issue.id, {
        content,
  ...
  const handleStartEdit = (c: BalanceCommentNodeModel) => {
    if (!isLoggedInNow()) {
      setErrorMessage(null);
      setIsLoginModalOpen(true);
      return;
  ...
  const handleSubmitEdit = async (commentId: string | number) => {
    ...
    try {
      await issueApi.updateComment(issue.id, commentIdNum, content);
      await refreshComments(issue.id);
  ...
  const handleToggleLike = async (commentId: string | number) => {
    ...
    const prevIssue = issue;
    const prevComments =
      ((prevIssue as BalanceDetail & { comments?: BalanceCommentNodeModel[] }).comments ??
        []) as BalanceCommentNodeModel[];

    const patchLikes = (nodes: BalanceCommentNodeModel[]): BalanceCommentNodeModel[] =>
      nodes.map((n) => {
    ...
    setIssue({
      ...prevIssue,
      comments: patchLikes(prevComments),
    } as BalanceDetail);

    try {
      await issueApi.toggleCommentLike(prevIssue.id, commentIdNum);
