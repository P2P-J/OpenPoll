...
export function useBalanceList(isLoggedIn: boolean) {
  ...
  const refresh = useCallback(async () => {
    const data = await balanceApi.getIssueList();
    setIssues(data);
  }, []);
  ...
  const openEdit = async (issue: BalanceListItem) => {
    try {
      setErrorMessage(null);
      setModalMode("edit");
      const detail = await balanceApi.getIssueDetail(issue.id);
      setEditing(issue);
  ...
  const handleDelete = async (issue: BalanceListItem) => {
    ...
    try {
      ...
      setErrorMessage(null);
      await balanceApi.deleteIssue(issue.id);
      await refresh();
    } catch (e) {
      setErrorMessage(getErrorMessage(e));
    } finally {
  ...
  const handleSubmit = async (payload: {
    ...
    try {
      ...
      if (modalMode === "create") {
        await balanceApi.createIssue(payload);
      } else {
        if (!editing) throw new Error("수정 대상을 찾을 수 없습니다.");
        await balanceApi.updateIssue(editing.id, payload);
      }
