...
import { BalanceCommentNode } from "@/components/balance/BalanceCommentNode";
import type { ReactNode } from "react";
import type { BalanceCommentNodeModel, VoteSide } from "@/types/balanceComment.types";
...
interface Props {
  ...
  countAllComments: (nodes: BalanceCommentNodeModel[]) => number;
  openLoginModal: () => void;

  isAdmin: boolean;
  isMyComment: (c: BalanceCommentNodeModel) => boolean;
  ...
  keyOf: (v: string | number) => string;

  handleSubmitComment: () => void;
}

export function BalanceCommentSection(props: Props) {
  const {
    isLoggedIn,
    isLoggedInNow,
    selectedOption,
  ...
  const renderCommentNode = (c: BalanceCommentNodeModel, depth = 0): ReactNode => (
    <BalanceCommentNode
      c={c}
      depth={depth}
      ...
      <div className="relative">
            ...
            <div className="flex-1">
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <button
                  ...
                  disabled={isLoggedInNow() ? !selectedOption || !comment.trim() : false}
        ...
        <div className="space-y-4">
          {comments.length === 0 ? (
            ...
            comments.map((c) => <div key={keyOf(c.id)}>{renderCommentNode(c, 0)}</div>)
          )}
        </div>
      </div>
    </motion.div>
