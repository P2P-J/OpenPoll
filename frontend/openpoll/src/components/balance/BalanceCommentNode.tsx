...
interface Props {
  ...
  onStartEdit: (c: BalanceCommentNodeModel) => void;
  ...
  renderChild: (c: BalanceCommentNodeModel, depth: number) => ReactNode;
  keyOf: (v: string | number) => string;
  openLoginModal: () => void;
}

export function BalanceCommentNode(props: Props) {
  const {
    c,
    depth,
    isLoggedIn,
  ...
  return (
    <div className={isReply ? "relative" : ""} style={{ paddingLeft: isReply ? "32px" : "0" }}>
      {isReply && (
        <>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-2">
              ...
              >
                {isAgree === true ? "찬성" : isAgree === false ? "반대" : "미표기"}
              </span>
