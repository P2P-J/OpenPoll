...
import { Flame, CheckCircle, Users, ArrowRight, X, Pencil } from "lucide-react";
import type { BalanceListItem } from "@/types/balance.types";

type BalanceListItemExtended = BalanceListItem & {
  voted?: boolean | null;
};
...
export function BalanceCard({
  ...
  const agreePercentSafe = participantsSafe <= 0 ? 0 : Number(issue.agreePercent ?? 0);
  const disagreePercentSafe = participantsSafe <= 0 ? 0 : Math.max(0, 100 - agreePercentSafe);

  const isHotIssue = participantsSafe >= 3000;
  const showCompleted = isLoggedIn && (Boolean(issueEx.voted) || issue.myVote !== null);
                ...
                <div className="flex-1 flex flex-col justify-center">
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-2xl font-bold text-gray-600">{disagreePercentSafe}%</span>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="text-center p-3 bg-black/5 rounded-xl">
                      ...
                      <div className="text-lg font-bold text-black">{agreeCountSafe.toLocaleString()}명</div>
                    </div>

                    <div className="text-center p-3 bg-black/5 rounded-xl">
                      <div className="text-xs text-gray-600 font-semibold mb-1">반대</div>
                      <div className="text-lg font-bold text-gray-600">{disagreeCountSafe.toLocaleString()}명</div>
                    </div>
                  </div>
                </div>

                <div className="flex items-center justify-between pt-4 border-t border-gray-300 mt-4">
                  <div className="text-xs sm:text-sm text-gray-600 font-medium">마우스를 떼면 다시 뒤집혀요</div>
