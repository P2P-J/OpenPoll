...
> | **재배포 가이드** | [docs/OpenPoll*AWS*종료\_가이드.pdf](docs/OpenPoll_AWS_종료_가이드.pdf) |
>
...
회원·투표·SSE 실시간 통계·AI 뉴스 요약·정치 성향 테스트(DOS)·밸런스 게임·포인트·출석·공유 카드까지 — **데브코스 8기 팀(6명)이 약 3.5개월에 걸쳐 구축**한 정치 콘텐츠 플랫폼입니다. 본인은 **프론트엔드 전반과 실시간 채팅(SSE)을 담당**했고, **팀 해산 후 약 2개월간 단독으로 운영·유지보수·종료**를 맡았습니다.
...
| 기여자        | 데브코스 8기 팀 6명 → 2026년 4월부터 단독 운영   |
...
OpenPoll 은 **데브코스 풀스택 8기의 팀 프로젝트(6명)로 시작**해 MVP 를 만들었고, **2026년 4월부터는 단독으로 운영·유지보수·종료**를 진행했습니다.

#### 팀 프로젝트 기간 — 팀원별 주 담당 (git 커밋 기록 기준)

| 팀원                                        | 주 담당 영역                                                                     |
| ------------------------------------------- | -------------------------------------------------------------------------------- |
| **조보근 ([@Aen](https://github.com/Aen))** | 프론트엔드 전반, 다크모드/디자인 시스템, 실시간 채팅(SSE)                        |
| 박찬영(tmakdrl)                             | 백엔드 코어(인증·투표·포인트·밸런스·DOS·대시보드), 초기 서버 셋업, 배포 워크플로 |
| 김정철                                      | 뉴스 크롤러 + OpenAI 요약 (BullMQ 큐)                                            |
| 김재우                                      | 프론트엔드 (DOS·뉴스·홈 페이지 기여)                                             |
| nicephj95                                   | 인증 화면, 밸런스 게임                                                           |

#### 본인(조보근)이 직접 담당한 영역

- **프론트엔드 전반** (React 19 + TypeScript) — 홈·DOS·밸런스·뉴스·프로필·블로그 등 대부분 페이지의 원저자. 구체 작업:
  - **DOS 결과/공유** — DosResultCard 컴포넌트, 결과 페이지 가로형 개편 + 키워드 태그, 결과 카드 PNG 다운로드(html-to-image, 1080×1080), 공유 모달 리디자인, 16개 유형 animal/color/tagline 필드
  - **블로그·홈** — 정치 교양 블로그 10편 신설, Bottom Navigation 블로그 탭 + 홈 블로그 섹션, 홈 FAQ 카드형 리디자인, 실시간 지지율 안내 UI
  - **인증 화면** — 회원가입 이메일 인증코드 UI, 회원탈퇴 모달, 비밀번호 변경 모달, 닉네임 유효성 검증, OAuth 유저 분기 처리
  - **밸런스·프로필** — 밸런스 상세 UI 개선·비로그인 투표 로그인 모달, 프로필/보안 설정 화면
  - **공통·품질** — 푸터 문의하기 모달, Modal 포커스 버그 수정, `STORAGE_KEYS` 상수 통합, 메모리 누수·불필요 리렌더링 개선, Atomic Design 디자인 시스템 리팩토링
- **다크모드 전면 도입 (단독)** — CSS 변수 기반 `design-tokens` + 시맨틱 `utilities` + `.dark` 클래스 swap
- **디자인 시스템** — CVA + Atomic Design + Tailwind v4 정적 빌드 환경 위 자체 토큰 시스템
- **실시간 전체 채팅 (SSE) — 풀스택 직접 구현** — 백엔드 브로드캐스트 + 프론트 위젯 (욕설 필터, 자동 재연결, 리사이즈)
- **인증 기능 확장** — 이메일 인증코드, 회원탈퇴, 비밀번호 변경
- **버그·품질 개선** — 투표 2표 중복 카운트 수정, 대시보드 집계 방식 개선, Rate Limiting·보안 하드닝
- **분석 / SEO / 광고** — GA4, GTM, MS Clarity, AdSense, sitemap, **OG 이미지 satori 빌드타임 정적 생성**
- **단독 운영기 (4~5월)** — AWS 인프라 **운영**(초기 구축은 팀), ElastiCache→로컬 Redis 비용 최적화, SEO·콘텐츠·블로그 보강, 종료 가이드 작성 및 서비스 종료

> ℹ️ **초기 AWS 인프라(EC2/RDS/ALB/Route 53)와 배포 파이프라인(GitHub Actions → SSM)은 팀 프로젝트 기간에 구축**되었으며, 이후 **운영·비용관리·종료는 단독**으로 담당했습니다.
...
2026-03  ┃ 밸런스 게임 + 댓글/대댓글/좋아요, 뉴스 크롤러 + OpenAI 요약 BullMQ 큐
   ...
   ↓
2026-04  ┃ [단독 운영 시작]
   ↓     ┃ AWS 인프라 운영·안정화 — ALB 헬스체크 경로 수정, nginx default block 충돌 해결
         ┃ 출석체크 UI 개선, OG 이미지 satori 동적 생성, 결과 카드 리디자인
...
### 5. SSM 기반 운영 적응 (단독 운영기)

EC2 가 private subnet 에 격납되어 일반 SSH 가 불가능했다. 단독 운영을 맡은 뒤 모든 운영 명령을 **AWS SSM Session Manager** 로 수행하는 데 적응했다. 배포 파이프라인(**GitHub OIDC → IAM Role → SSM Send Command**, 비밀키 없는 배포)은 팀 프로젝트 기간에 구축된 것으로, 이후 운영·유지보수를 담당했다.
...
> 아래는 **프로젝트 전체 성과**이며, 본인 직접 기여 항목은 별도로 표기했습니다.

- 정치 콘텐츠 풀스택 플랫폼의 **MVP 완성 → 실서비스 배포 → 약 4개월 운영** 사이클 완주 _(팀)_
- 도메인 `openpoll.co.kr` 위에 **HTTPS + Route 53 + ALB + GitHub Actions 자동 배포** _(팀 구축 · 단독 운영)_
- **[본인]** GA4 / GTM / MS Clarity 연동으로 트래픽 분석 환경 구축
- OpenAI API 기반 뉴스 자동 요약 파이프라인 (BullMQ 워커, 동시성 3) _(뉴스 담당: 김정철 · 단독 운영기 유지)_
- **[본인]** 다크모드 + 시맨틱 디자인 토큰 시스템 직접 설계 및 전체 페이지에 적용
- **[본인]** 실시간 전체 채팅(SSE) 풀스택 구현 · OG 이미지 satori 동적 생성 · 투표 이중카운트 버그 수정
...
종료 결정 자체보다 더 중요한 것은 **종료의 방식**이었다. 단순히 인스턴스를 꺼버리면 EBS · EIP · Detached 리소스가 보이지 않는 곳에서 계속 과금된다. 그래서 종료 전 마지막 작업으로 [docs/OpenPoll*AWS*종료\_가이드.pdf](docs/OpenPoll_AWS_종료_가이드.pdf) 를 작성했다. 무엇을 왜 삭제해야 하는지, 무엇을 백업해야 하는지, 그리고 **언제든 다시 같은 인프라로 올릴 수 있는 12단계 재배포 가이드**까지 포함했다.
...
> 다시 같은 인프라로 올리는 방법은 [docs/OpenPoll*AWS*종료\_가이드.pdf](docs/OpenPoll_AWS_종료_가이드.pdf) 5장 "재배포 가이드" 를 참고하세요.
